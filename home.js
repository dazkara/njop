...
items1.forEach(item => {
    const checkbox = item.querySelector("input[type='checkbox']"); // Menemukan elemen checkbox di dalam setiap item
    const itemText = item.querySelector(".item-text"); // Menemukan elemen teks di dalam setiap item

    checkbox.addEventListener("change", () => { // Menggunakan event "change" pada checkbox untuk menanggapi perubahan status centang
        item.classList.toggle("checked1"); // Menambah atau menghapus kelas "checked1" saat checkbox dicentang atau tidak
...
items2.forEach(item => {
    ...
    checkbox.addEventListener("change", () => { // Menggunakan event "change" pada checkbox untuk menanggapi perubahan status centang
        item.classList.toggle("checked2"); // Menambah atau menghapus kelas "checked2" saat checkbox dicentang atau tidak
...
items1.forEach(item => {
    ...
    checkbox.addEventListener("change", () => { // Menggunakan event "change" pada checkbox untuk menanggapi perubahan status centang
        item.classList.toggle("checked3"); // Menambah atau menghapus kelas "checked1" saat checkbox dicentang atau tidak
...
function toggleLayer(layerName) {
    // Tentukan URL GeoJSON berdasarkan nama layer
    var geojsonUrl;
    var styleOptions = {};
    var layer = null;
    ...
    switch (layerName) {
        case 'estimasiNJOP':
            ...
            styleOptions = {
                attribute: 'estimasi', // Nama atribut untuk memetakan warna
                colorScale: chroma.scale('YlOrBr').domain([167912, 1416940]), // Skala warna untuk rentang estimasi
                fillColor: '#8c510a', // Warna default
                fillOpacity: 0.7
            };
            break;
        case 'fasilitasUmum':
        ...
        case 'bufferSungai':
            ...
            styleOptions = {
                attribute: 'distance', // Nama atribut untuk memetakan warna
                colorScale: chroma.scale(['lightblue', 'darkblue']).domain([0, 1000]), // Skala warna untuk rentang estimasi
                fillColor: '#2c8bc7', // Warna default
                fillOpacity: 0.7
            };
            break;
        ...
        case 'bufferMakam':
            ...
            styleOptions = {
                attribute: 'distance', // Nama atribut untuk memetakan warna
                colorScale: chroma.scale('OrRd').domain([0, 1000]), // Skala warna untuk rentang distance
                fillColor: '#d73027', // Warna default
                fillOpacity: 0.7
            };
            break;
        default:
    ...
    if (isChecked) {
        // Jika checkbox dicentang, muat GeoJSON dan tambahkan ke peta
        fetch(geojsonUrl)
            .then(response => response.json())
            .then(data => {
                // Tambahkan layer GeoJSON ke peta dengan styling yang sesuai
                layer = L.geoJSON(data, {
                    ...
                    onEachFeature: function(feature, layer) {
                        if (layerName === 'estimasiNJOP') {
                            // Format nilai estimasi dengan pemisah titik dan tambahan 'Rp.'
                            ...
                            // Tentukan ikon berdasarkan keterangan
                            var iconUrl = 'gambar/' + Keterangan + '.png';
                            // Tambahkan marker dengan ikon yang sesuai
                ...
                layer.addTo(map);
                if (layerName === 'estimasiNJOP') {
                    // Tambahkan legenda untuk layer estimasiNJOP
...
function createLegend(colorScale, title, attribute) {
    ...
    legendControl.onAdd = function(map) {
        ...
        // Ambil rentang nilai dari skala warna
        var domain = colorScale.domain();

        if (title === 'Estimasi NJOP') {
            // Jika layer adalah estimasiNJOP, buat legenda berdasarkan interval warna
            ...
            // Jika layer adalah bufferMakam atau bufferSungai, buat legenda berdasarkan range buffer
            ...
            // Jika layer adalah bufferSungai, buat legenda berdasarkan range buffer
            ...
            // Jika layer adalah bufferJalanArteri, buat legenda berdasarkan range buffer
            ...
            // Jika layer bukan estimasiNJOP, bufferMakam, atau bufferSungai, buat legenda berdasarkan range buffer
...
// Fungsi untuk menambahkan titik ribuan pada angka
